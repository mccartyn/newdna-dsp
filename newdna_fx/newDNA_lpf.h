#include <cassert>
#include <../rewrite/juce_HeapBlock.h>
#include <../newdna_dsp/filters/comb.h>
#include <../newdna_dsp/filters/allpass.h>
...
public:
    ...
    */
    void setParameters (const Parameters& newParams)
    {
        ...
        dryGain =  (newParams.dryLevel * dryScaleFactor);
        //wetGain1.setTargetValue (0.5f * wet * (1.0f + newParams.width));
        wetGain1 = (0.5f * wet * (1.0f + newParams.width));
        //wetGain2.setTargetValue (0.5f * wet * (1.0f - newParams.width));
        wetGain2 = (0.5f * wet * (1.0f - newParams.width));
    ...
    */
    void setSampleRate (const double sampleRate)
    {
        ...
        damping = (float)(smoothTime * sampleRate);
        feedback =(float)(smoothTime * sampleRate); 
        dryGain =  (float)(smoothTime * sampleRate);
        wetGain1 = (float)(smoothTime * sampleRate);
        wetGain2 = (float)(smoothTime * sampleRate);
    }
    ...
    void processStereo (float* const left, float* const right, const int numSamples) noexcept
    {
        ...
        for (int i = 0; i < numSamples; ++i)
        {
            ...
            const float damp    = damping;//damping.getNextValue();
            const float feedbck = feedback;//feedback.getNextValue();
            ...
            const float dry  = dryGain;//dryGain.getNextValue();
            const float wet1 = wetGain1;//wetGain1.getNextValue();
            const float wet2 = wetGain2;//wetGain2.getNextValue();
...
private:
    ...
    void setDamping (const float dampingToUse, const float roomSizeToUse) noexcept
    {
        ...
        damping = dampingToUse;
        //feedback.setTargetValue (roomSizeToUse);
        feedback = roomSizeToUse;
    }
    ...
    //TODO: in other examples these parameters are using a class that uses some "smoothing" or linear interpolation techniques to avoid audio glitches
    float damping, feedback, dryGain, wetGain1, wetGain2;
